...
| 連線狀態訊息 | 走路 / 巡迴 / 畫圖 | 注入失敗時顯示重試進度、放棄時顯示原因，詳見[走路中斷與復原](#走路中斷與復原) |
...
Helper 跟 iPhone 之間的連線**本來就會偶爾斷一下再自己接回來**——USB 抖一下、iPhone
鎖定、pymobiledevice3 重建 tunnel 都會。helper 端對這件事早有準備（`Helper.retryLoop`：
子程序死掉 → 清乾淨 → 等 5 秒 → 重建 tunnel 跟 dvt），實測日誌平均一天會發生兩三次。
...
每秒送一格 GPS ──┬─ 成功 ─────────────→ 走下一格
                 └─ 失敗 ─→ 原地重試（不前進、不改座標）
                              ├─ helper 接回來了 → 從中斷的同一格繼續走
                              └─ 撐滿 3 分鐘還不行 → 停止走路 + 面板顯示原因
```

**重試期間刻意不推進路徑**。等 helper 恢復之後從中斷的同一點接回去，不會為了補進度
一次瞬移一大段——那才是真的會被伺服器端的行為模型抓。

面板上會看到：

| 顯示 | 意思 |
|---|---|
| 橘字「連線中斷、重試中…（第 n 次、已等 m 秒）」 | 正在撐，走路沒停，等就好 |
| 紅字「與 Helper 的連線中斷超過 3 分鐘仍未恢復…」 | 真的放棄了，去看 iPhone 還接著沒 |

### 走到一半自己停了

先看面板有沒有紅字。**沒有紅字就不是連線問題**，是使用者操作（按了停止 / 清空 /
恢復真實 GPS）或佇列被改動。有紅字的話翻 helper 日誌對時間：

```bash
sudo tail -n 200 /Library/Logs/locspoof-helper.err.log
```

找 `[retry] subprocess died` 跟 `[inject] response timeout` 這兩行。

> 舊版行為（已修）：**一次注入失敗就直接終止整段走路**，而且靜默、什麼都不顯示。
> helper 明明幾秒後就自己接回來了，走路卻永遠不會回來——「巡迴走一走無預警停掉」
> 就是這麼來的。
